...
def my_simulator_ss_noise(theta):
    ...
    def solve_ode_t(params, t):
        ...
        solution = odeint(modelg, initial_conditions, t, args=(params,))
        noise = solution * 0.05
...
true_data = my_simulator_ss_noise(true_params_list[0]) # Generate observations,with data from 100 timepoints for each of 6 variables
simulator_wrapper, prior = prepare_for_sbi(my_simulator_ss, prior)
...
posterior_samples_cnn1 = posterior.sample((5000,), x=true_data)
_ = analysis.pairplot(
    posterior_samples_cnn1, limits=[[-100, 300], [-100, 300], [-100, 300], [20, 40], [20, 40], [20, 40]], figsize=(6, 6) #It should be close to 247, but since this is a toy example we are not expecting it to work (just checking it runs).
)

plt.savefig(f'6p-ss_set1.png')

### Save csv...
column_name = ['k1','k2','k3','a1','a2','a3']
cnn1_2r1k3p = pd.DataFrame(data=posterior_samples_cnn1, columns=column_name)
cnn1_2r1k3p.to_csv('ss_6p_set1.csv')
...
true_data = my_simulator_ss_noise(true_params_list[1]) # Generate observations,with data from 100 timepoints for each of 6 variables
simulator_wrapper, prior = prepare_for_sbi(my_simulator_ss, prior)
...
plt.savefig(f'6p-ss_set2.png')

### Save csv...
column_name = ['k1','k2','k3','a1','a2','a3']
cnn1_2r1k3p = pd.DataFrame(data=posterior_samples_cnn1, columns=column_name)
cnn1_2r1k3p.to_csv('ss_6p_set2.csv')
...
for param in all_params:
    if param not in headers:
        new_param_dict[param] = param_dict[param][2]
    elif param in headers:
        new_param_dict[param] = param
...
true_data = my_simulator_ss_noise(true_params_list[2]) # Generate observations,with data from 100 timepoints for each of 6 variables
simulator_wrapper, prior = prepare_for_sbi(my_simulator_ss, prior)
...
plt.savefig(f'6p-ss_set3.png')

### Save csv...
column_name = ['k1','k2','k3','a1','a2','a3']
cnn1_2r1k3p = pd.DataFrame(data=posterior_samples_cnn1, columns=column_name)
cnn1_2r1k3p.to_csv('ss_6p_set3.csv')
...
for param in all_params:
    if param not in headers:
        new_param_dict[param] = param_dict[param][3]
    elif param in headers:
        new_param_dict[param] = param
...
true_data = my_simulator_ss_noise(true_params_list[3]) # Generate observations,with data from 100 timepoints for each of 6 variables
simulator_wrapper, prior = prepare_for_sbi(my_simulator_ss, prior)
## Try to preserve the relationships of the data with the same kernel size of 3...
...
plt.savefig(f'6p-ss_set4.png')

### Save csv...
column_name = ['k1','k2','k3','a1','a2','a3']
cnn1_2r1k3p = pd.DataFrame(data=posterior_samples_cnn1, columns=column_name)
cnn1_2r1k3p.to_csv('ss_6p_set4.csv')
...
true_data = my_simulator_ss_noise(true_params_list[4]) # Generate observations,with data from 100 timepoints for each of 6 variables
simulator_wrapper, prior = prepare_for_sbi(my_simulator_ss, prior)
...
plt.savefig(f'6p-ss_set5.png')

### Save csv...
column_name = ['k1','k2','k3','a1','a2','a3']
cnn1_2r1k3p = pd.DataFrame(data=posterior_samples_cnn1, columns=column_name)
cnn1_2r1k3p.to_csv('ss_6p_set5.csv')