...
#generate an empty np array to store simulated data
    ...
    for i in range(num_samples):
        params_val= np.random.default_rng().uniform(0.01,250,2).flatten() # np.concatenate((np.random.default_rng().uniform(0.01,250,3),np.random.default_rng().uniform(20.,40.,3))).flatten()
...
### now do snpe 3k samples for 5 rounds
...
posterior_samples2r = posterior.sample((5000,), x=x_o)

### Save plot...
_ = analysis.pairplot(
    posterior_samples2r, limits=[[-100, 300], [-100, 300]], figsize=(6, 6) #It should be close to 247, but since this is a toy example we are not expecting it to work (just checking it runs).
)

plt.savefig(f'ss2p2r5k.png')

### Save csv...
column_name = ['k1','k2']
snpess3p2r = pd.DataFrame(data=posterior_samples2r, columns=column_name)
snpess3p2r.to_csv('ss2p2r5k.csv')
...
#Save posterior samples
posterior_samples2r = posterior.sample((5000,), x=true_data)
