...
### Save plot...
_ = analysis.pairplot(
    posterior_samples_RNN, limits=[[-100, 300], [-100, 300]], figsize=(6, 6) #It should be close to 247, but since this is a toy example we are not expecting it to work (just checking it runs).
)

plt.savefig(f'snpe_RNN_2p2r5k.png')

### Save csv...
column_name = ['k1','k2']
snpernn3p5r = pd.DataFrame(data=posterior_samples_RNN, columns=column_name)
snpernn3p5r.to_csv('snpe_RNN_2p2r5k.csv')