...
#generate an empty np array to store simulated data
    ...
    for i in range(num_samples):
        params_val= np.random.default_rng().uniform(0.01,250,6).flatten() # np.concatenate((np.random.default_rng().uniform(0.01,250,3),np.random.default_rng().uniform(20.,40.,3))).flatten()
...
true_data = my_simulator_ss_noise(true_params) # Generate observations,with data from 100 timepoints for each of 6 variables
...
neural_posterior = utils.posterior_nn(
    model="maf", embedding_net=embedding_net_x, hidden_features=50, num_transforms=3)
