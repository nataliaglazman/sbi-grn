...
neural_posterior = utils.posterior_nn(
    model="maf", embedding_net=embedding_net2, hidden_features=50, num_transforms=3)
...
posterior_samples_cnn2 = posterior.sample((5000,), x=true_data)
_ = analysis.pairplot(
    posterior_samples_cnn2, limits=[[-100, 300], [-100, 300], [-100, 300]], figsize=(6, 6) #It should be close to 247, but since this is a toy example we are not expecting it to work (just checking it runs).
)

plt.savefig(f'3p-2r-1500s-CNN1pad1.png')

### Save csv...
column_name = ['k1','k2','k3']
cnn2_2r1k3p = pd.DataFrame(data=posterior_samples_cnn2, columns=column_name)
cnn2_2r1k3p.to_csv('cnn1pad1_2r1k3p.csv')
...
param_dict = {'k1': 200, 'k2': 200, 'k3': 200, 'n1': 5, 'n2': 5, 'n3': 5, 'dm1': 1.143402097500176, 'dm2': 1.143402097500176, 'dm3': 1.143402097500176, 'dp1': 0.7833664565550977, 'dp2': 0.7833664565550977, 'dp3': 0.7833664565550977,
              'a1': 24.78485282457379, 'a2': 24.78485282457379, 'a3': 24.78485282457379, 'g1': 0.024884149937163258, 'g2': 0.024884149937163258, 'g3': 0.024884149937163258, 'b1': 33.82307682700831, 'b2': 33.82307682700831, 'b3': 33.82307682700831}
...
plt.savefig(f'3p-2r-1500s-CNN1pad1_200.png')

### Save csv...
column_name = ['k1','k2','k3']
cnn2_2r1k3p = pd.DataFrame(data=posterior_samples_cnn2, columns=column_name)
cnn2_2r1k3p.to_csv('cnn1pad1_2r1k3p_200.csv')
