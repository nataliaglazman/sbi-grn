...
def my_simulator(theta):
    ...
    def solve_ode(theta, t):
        ...
        yf = y.flatten() #6 timepoints x 100...
        return torch.tensor(yf, dtype=torch.float32)
...
true_solutions_unflattened = unflatten(true_data)
...
posterior_samples2r = posterior.sample((10000,), x=true_data)

### Save plot...
_ = analysis.pairplot(
    posterior_samples2r, limits=[[-100, 300], [-100, 300], [-100, 300]], figsize=(6, 6) #It should be close to 247, but since this is a toy example we are not expecting it to work (just checking it runs).
)
...
snpeflat3p2r = pd.DataFrame(data=posterior_samples2r, columns=column_name)
snpeflat3p2r.to_csv('snpeflat3p2r.csv')
...
#Save posterior samples
posterior_samples5r = posterior.sample((10000,), x=true_data)
### Save plot...
_ = analysis.pairplot(
    posterior_samples5r, limits=[[-100, 300], [-100, 300], [-100, 300]], figsize=(6, 6) #It should be close to 247, but since this is a toy example we are not expecting it to work (just checking it runs).
)
...
### Save csv...
column_name = ['k1','k2','k3']
snpeflat3p5r = pd.DataFrame(data=posterior_samples5r, columns=column_name)
snpeflat3p5r.to_csv('snpeflat3p5r.csv')

######### NOW THE SAME BUT FOR 6 PARAMETERS ##########
...
def my_simulator(theta):
    ...
    def solve_ode(theta, t):
        initial_conditions = np.array([0, 2, 0, 1, 0, 3], dtype=np.float32)
        y = odeint(model, initial_conditions, t, args=(theta,))
        yt = torch.tensor(y, dtype=torch.float32).flatten()
        return yt #we return flattened but 100x6...
    return solve_ode(theta, t)
...
true_params = tuple(param_dict[parameter] for parameter in headers)
true_data = my_simulator(true_params) ### IT WILL BE UNSQUEEZED...
true_solutions_unflattened = unflatten(true_data)
...
### Save plot...
_ = analysis.pairplot(
    posterior_samples2r, limits=[[-100, 300], [-100, 300], [-100, 300], [20,40], [20,40], [20,40]], figsize=(6, 6) #It should be close to 247, but since this is a toy example we are not expecting it to work (just checking it runs).
)
...
snpeflat6p2r = pd.DataFrame(data=posterior_samples2r, columns=column_name)
snpeflat6p2r.to_csv('snpeflat6p2r.csv')
...
### Save csv...
column_name = ['k1','k2','k3', "a1", "a2", "a3"]
snpeflat6p5r = pd.DataFrame(data=posterior_samples5r, columns=column_name)
snpeflat6p5r.to_csv('snpeflat6p5r.csv')
