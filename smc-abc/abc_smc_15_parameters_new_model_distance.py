...
from scoring_15_pars_new_model_distance import score
...
def score_wrapper(log_k1: float,
                log_k2: float, log_k3: float, log_g1: float, log_g2: float, log_g3: float, log_a1: float, log_a2: float, log_a3: float, log_n1: float, log_n2: float, log_n3: float, log_b1: float, log_b2: float, log_b3: float) -> float:
...
def make_output_folder(name: str = "smc_15_pars_new_model") -> None:
...
def sequential_abc(initial_dist: float = 700.0,
    ...
    if prior_label is None:
        ...
        pars = np.loadtxt(f'smc_15_pars_new_model/pars_{prior_label}.out')
        weights = np.loadtxt(f'smc_15_pars_new_model/weights_{prior_label}.out')
        accepted_distances = np.loadtxt(f'smc_15_pars_new_model/distances_{prior_label}.out')
    ...
    while not_converged:
        ...
        np.savetxt(f'smc_15_pars_new_model/pars_{label}.out', pars)  # type: ignore
        np.savetxt(f'smc_15_pars_new_model/weights_{label}.out', weights)  # type: ignore
        np.savetxt(f'smc_15_pars_new_model/distances_{label}.out', accepted_distances)
