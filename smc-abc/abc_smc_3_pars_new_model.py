...
from scoring_3_pars_new_model import score
...
def make_output_folder(name: str = "smc_3_pars_new_model") -> None:
...
def sequential_abc(initial_dist: float = 500.0,
    ...
    if prior_label is None:
        ...
        pars = np.loadtxt(f'smc_3_pars_new_model/pars_{prior_label}.out')
        weights = np.loadtxt(f'smc_3_pars_new_model/weights_{prior_label}.out')
        accepted_distances = np.loadtxt(f'smc_3_pars_new_model/distances_{prior_label}.out')
    ...
    while not_converged:
        ...
        np.savetxt(f'smc_3_pars_new_model/pars_{label}.out', pars)  # type: ignore
        np.savetxt(f'smc_3_pars_new_model/weights_{label}.out', weights)  # type: ignore
        np.savetxt(f'smc_3_pars_new_model/distances_{label}.out', accepted_distances)
